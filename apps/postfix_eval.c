...
int eval_postfix(const char *expr, long *res)
{
    ...
    while(expr[i] != '\0'){
        if(isdigit(expr[i])){
            ...
            number = expr[i];
            *int_ptr = number-48;
            upo_stack_push(stack, int_ptr);
        }
        else if(expr[i] != ' '){
            operator = expr[i];
            if(operator == '-' || operator == '+' || operator == '*' || operator == '/' || operator == '^' || operator == '%'){
                op2 = *((int *)upo_stack_top(stack));
                upo_stack_pop(stack, 1);
                op1 = *((int *)upo_stack_top(stack));
                upo_stack_pop(stack, 1);
            }
    ...
    if(upo_stack_size(stack) == 1){
        ...
        *res = *((int *)upo_stack_top(stack));
        upo_stack_pop(stack, 1);
        upo_stack_destroy(stack, 1);
        return 1;
    }
    else{
        fprintf(stderr, "Malformed Expressio!n\n");
        return 0;
    }
